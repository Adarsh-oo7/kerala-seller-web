...
// // âœ… Local + Production (same as BuyerLogin)
...
//   // âœ… FIXED: Enhanced shop context detection with proper priority
...
//     // âœ… CRITICAL FIX: Priority 1 - Query parameter ?id=xxx (highest priority for your case)
...
//   // âœ… ENHANCED: Fixed wishlist fetching with multiple fallback methods
...
//       // âœ… ENHANCED: Parse wishlist count from various response formats
//       let count = 0;
//       if (wishlistData) {
...
//       // âœ… FIXED: Only add store filter if we have a valid numeric store ID
...
//         const dataPromises = [
//           fetchWishlistCount(headers),  // âœ… Enhanced wishlist fetching
//           fetchOrdersCount(headers)
//         ];

//         // âœ… FIXED: Only fetch store data if we have a valid numeric store ID
//         if (storeInfo.isInStore && storeInfo.storeId && /^\d+$/.test(storeInfo.storeId)) {
...
//   // âœ… FIXED: Enhanced authentication and redirect logic
// useEffect(() => {
...
//     // âœ… User is authenticated, proceed with fetching profile
//     await fetchProfile();
//   };
...
//                 {ordersCount > 0
//                   ? `${ordersCount} order${ordersCount !== 1 ? 's' : ''} â€¢ Track and manage`
...
//             ðŸª Shop ID: {shopContext.shopId} â€¢ Method: {shopContext.detectionMethod}
//           </p>
//         )}
//       </div>
//     );
...
//                   {storeData ? (
//                     <>You're shopping at <strong>{storeData.name}</strong> â€¢ Store ID: {shopContext.shopId}</>
//                   ) : (
//                     <>Store ID: {shopContext.shopId} â€¢ Detection: {shopContext.detectionMethod}</>
//                   )}
//                 </span>
//               </div>
//             </div>
...
//                   {buyer.phone_verified ? 'âœ… Verified' : 'âš ï¸ Pending'}
//                 </span>
//               </div>
//               <div style={styles.summaryItem}>
...
import { useState, useEffect, useRef } from 'react';
import api from '../../app/lib/api'; // ✅ FIXED: use configured api instance instead of raw axios
import { useRouter } from 'next/navigation';
import Header from '../../components/common/Header';
import Footer from '../../components/common/Footer';
import "../../styles/KeralasellersProfile.css";
...
const PROFILE_API = `/api/buyer/profile/`;       // ✅ relative — api instance has baseURL
...
export default function ProfilePage() {
  ...
  // ✅ REMOVED: getAuthHeaders() — api interceptor handles Authorization header automatically
  ...
  const fetchProfile = async (showRefreshing = false) => {
    // ✅ FIXED: no getAuthHeaders() needed — api interceptor handles it
    // ✅ FIXED: token check is done in useEffect before calling this function
    if (hasFetchedRef.current && !showRefreshing) {
      return;
    }
    ...
    try {
      const response = await api.get(PROFILE_API, { timeout: 15000 }); // ✅ api.get
      ...
      if (!hasFetchedRef.current || showRefreshing) {
        const dataPromises = [
          fetchWishlistCount(),  // ✅ no headers arg needed
          fetchOrdersCount()     // ✅ no headers arg needed
        ];

        if (storeInfo.isInStore && storeInfo.storeId && /^\d+$/.test(storeInfo.storeId)) {
          dataPromises.push(fetchStoreData(storeInfo.storeId)); // ✅ no headers arg needed
        }
  ...
  // ✅ FIXED: check token in localStorage before fetching — redirect if missing
  useEffect(() => {
    const checkAuthAndFetchProfile = async () => {
      const token = typeof window !== 'undefined'
        ? localStorage.getItem('access_token') || localStorage.getItem('buyerAccessToken')
        : null;
      ...
      // ✅ Token exists — api interceptor will attach it automatically
      await fetchProfile();
    };

    checkAuthAndFetchProfile();
  }, []); // ✅ empty deps — run once on mount only, avoids double-fetch
