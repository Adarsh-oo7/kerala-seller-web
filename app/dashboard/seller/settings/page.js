...
export default function SettingsPage() {
  ...
  const checkCashfreeStatus = async () => {
    ...
    if (!token) return;
    try {
      const response = await axios.get(`${API_BASE_URL}/api/payments/bank-account/status/`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.registered || response.data.is_verified) {
        setCashfreeConnected(true);
      }
    } catch (error) {
      console.log('No bank account registered yet');
    }
  };
  ...
  const handleDeleteLogo = async () => {
    ...
    setErrorMessage('');
    try {
      const response = await axios.patch(API_URL, { cloudinary_logo: null }, { headers });
  ...
  const handleDeleteBanner = async () => {
    ...
    setErrorMessage('');
    try {
      const response = await axios.patch(API_URL, { cloudinary_banner_1: null }, { headers });
      if (response.data.success !== false) {
        setCurrentBannerUrl('');
        setBannerPreview('');
        setNewBanner(null);
  ...
  // ✅ UPDATED: New handleCashfreeConnect function
  const handleCashfreeConnect = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/payments/bank-account/add/`,
        {
          account_holder_name: store.cashfree_account_holder,
          account_number: store.cashfree_bank_account,
          ifsc_code: store.cashfree_ifsc,
      ...
      if (response.status === 201) {
        setCashfreeConnected(true);
        setSuccessMessage('✅ Bank account registered! 100% of sales go directly to you.');
        setTimeout(() => setSuccessMessage(''), 5000);
      }
    } catch (error) {
      ...
      setErrorMessage(
        ...
        '❌ Bank registration failed. Please try again.'
      );
  ...
  return (
    <div style={s.c}>
      ...
      <form onSubmit={handleSubmit} style={s.f}>
        ...
        <div style={s.sec}>
          <h3 style={s.sh}><CreditCard size={18}/>Payment Methods</h3>
          ...
          {/* ✅ CASHFREE SECTION - UPDATED */}
          {store.paymentmethod==='CASHFREE' && <div style={s.cs}>{!cashfreeConnected?(<><div style={s.ci}><AlertCircle size={14}/><span><strong>0% Fee!</strong> Register your bank account for 100% sales directly.</span></div><div style={s.gr}><div style={s.fg}><label style={s.l}>Bank Account Number *</label><input type="text" name="cashfree_bank_account" value={store.cashfree_bank_account} onChange={handleInputChange} required style={s.in} placeholder="1234567890"/></div><div style={s.fg}><label style={s.l}>IFSC Code *</label><input type="text" name="cashfree_ifsc" value={store.cashfree_ifsc} onChange={handleInputChange} required style={s.in} placeholder="HDFC0001234"/></div></div><div style={s.fg}><label style={s.l}>Account Holder Name *</label><input type="text" name="cashfree_account_holder" value={store.cashfree_account_holder} onChange={handleInputChange} required style={s.in} placeholder="Name as per bank"/></div><div style={s.bb}><Check size={16} color="#10b981"/><div><strong>Why use Cashfree?</strong><ul style={{margin:'6px 0 0 0',paddingLeft:'16px',fontSize:'13px'}}><li>✅ 0% Commission - You keep 100%</li><li>✅ Direct bank transfers</li><li>✅ Automatic payouts</li><li>✅ No hidden charges</li></ul></div></div><button type="button" onClick={handleCashfreeConnect} disabled={isConnectingCashfree} style={s.cb}>{isConnectingCashfree?'⏳ Registering...':'🏦 Register Bank Account (0% Fee)'}</button></>) : (<div style={s.cc}><Check size={18} color="#10b981"/><div><strong>✅ Bank Account Registered!</strong><p style={{margin:'6px 0 0 0',fontSize:'13px',color:'#065f46'}}>100% of your sales will go directly to your bank account.</p></div></div>)}</div>}
