...
import { Shield, Upload, Check, AlertCircle, Star, Building, CreditCard, Save, ExternalLink, Eye, EyeOff, Image as ImageIcon } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const requestData = {
        ...
        cloudinary_logo: newLogo ? { public_id: newLogo.publicid, url: newLogo.url } : null,
        cloudinary_banner_1: newBanner ? { public_id: newBanner.publicid, url: newBanner.url } : null,
      };
        ...
        <div style={s.sec}>
          ...
          <div style={s.ig}>
            <div>
              ...
              <div style={s.iu}>
                {logoPreview || currentLogoUrl ? <img src={logoPreview || currentLogoUrl} alt="Logo" style={s.lp}/> : <div style={s.ph}><Upload size={20}/><span>No Logo</span></div>}
            ...
            <div>
              ...
              {currentBannerUrls.length>0 && <div style={{marginTop:'10px',marginBottom:'10px'}}><label style={s.l}>Selected ({currentBannerUrls.length})</label><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:'6px',marginTop:'6px'}}>{currentBannerUrls.map((url,i)=><div key={i} style={{position:'relative',borderRadius:'6px',overflow:'hidden',border:'2px solid #10b981'}}><img src={url} alt={`Banner ${i+1}`} style={{width:'100%',height:'70px',objectFit:'cover'}}/><div style={{position:'absolute',top:'3px',left:'3px',backgroundColor:'#10b981',color:'white',borderRadius:'3px',padding:'2px 5px',fontSize:'10px',fontWeight:600}}>#{i+1}</div></div>)}</div></div>}
              <div style={{marginTop:'10px'}}><label style={s.l}>Custom Banner{(bannerPreview || currentBannerUrl) && <span style={{color:'#10b981',fontSize:'11px',marginLeft:'6px'}}>✓ {bannerPreview ? 'New' : 'Set'}</span>}</label><div style={s.iu}>{bannerPreview || currentBannerUrl ? <img src={bannerPreview || currentBannerUrl} alt="Banner" style={s.bp}/> : <div style={s.ph}><Upload size={20}/><span>No Banner</span></div>}<div style={s.io}><input type="file" accept="image/*" onChange={(e)=>handleFileChange('banner',e.target.files[0])} style={s.hi} id="ban-up" disabled={isUploading}/><label htmlFor="ban-up" style={s.ub}>{isUploading?'Uploading...':(currentBannerUrl||bannerPreview)?'📷 Change':'📤 Upload'}</label></div></div>{bannerPreview && <p style={{fontSize:'11px',color:'#f59e0b',marginTop:'6px',fontWeight:600}}>⚠️ Click Save to apply</p>}</div>
            </div>
          </div>
        </div>
        <div style={s.sec}>
