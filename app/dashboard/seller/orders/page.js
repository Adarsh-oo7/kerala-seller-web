    ...
    <div style={styles.card}>
      <div style={styles.cardHeader}>
        <div style={styles.orderInfo}>
          ...
          <div style={styles.orderMeta}><Clock size={14} /><span>{formatDate(order.created_at)}</span></div>
        </div>
        <div style={styles.orderAmount}><strong style={styles.totalAmount}>₹{parseFloat(order.total_amount).toFixed(2)}</strong></div>
      </div>

      <div style={styles.cardBody}>
        <div style={styles.detailRow}>
          <div style={styles.detailItem}><User size={16} /><span>{order.customer_name || 'Guest Customer'}</span></div>
          {order.customer_phone && (<div style={styles.detailItem}><span style={styles.phoneNumber}>+91 {order.customer_phone}</span></div>)}
        </div>

        {order.payment_method && (
          <div style={styles.detailRow}>
            <div style={styles.detailItem}>{order.payment_method === 'ONLINE' ? <CreditCard size={16} /> : <Wallet size={16} />}<span>{order.payment_method === 'ONLINE' ? 'Online Payment' : 'Cash on Delivery'}</span></div>
            <div style={{...styles.statusBadge, ...getPaymentStatusStyle(order.payment_status)}}>{order.payment_status || (order.payment_method === 'ONLINE' ? 'PAID' : 'COD')}</div>
          </div>
        )}

        <div style={styles.itemsSection}>
          <h4 style={styles.itemsHeader}><ShoppingCart size={16} />Items ({order.items?.length || 0})</h4>
          {order.items && order.items.length > 0 ? (
            <ul style={styles.itemList}>
              ...
              {order.items.length > 3 && <li style={styles.moreItems}>...and {order.items.length - 3} more items</li>}
            </ul>
          ) : (
      ...
      <div style={styles.cardFooter}>
        <div style={{...styles.statusBadge, ...getStatusStyle(order.status)}}>{getStatusIcon(order.status)}<span>{order.status}</span></div>
        <Link href={`/dashboard/seller/orders/${order.id}`} style={styles.actionButton}><Eye size={16} /><span>View Details</span></Link>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={styles.pageContainer}>
      <div style={styles.header}>
        <div>
          <h1 style={styles.pageTitle}><Package size={28} />My Orders {orderStats.total && (<span style={styles.totalBadge}>({orderStats.total})</span>)}</h1>
        ...
        <div style={styles.headerActions}>
          ...
          <button onClick={handleExportOrders} style={styles.exportButton}><Download size={18} />Export CSV</button>
          <button onClick={fetchOrdersData} style={styles.refreshButton}><RefreshCw size={18} />Refresh</button>
        </div>
      </div>
...
const styles = {
  ...
  pageTitle: { fontSize: '2rem', fontWeight: '700', color: '#1f2937', margin: '0 0 8px 0', display: 'flex', alignItems: 'center', gap: '12px' },
  ...
  notificationContainer: { position: 'relative', cursor: 'pointer', padding: '8px', borderRadius: '8px', border: '1px solid #d1d5db', backgroundColor: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  ...
  notificationsDropdown: { position: 'absolute', top: '100%', right: '0', backgroundColor: 'white', border: '1px solid #d1d5db', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', width: '320px', zIndex: 1000, maxHeight: '400px', overflow: 'hidden', marginTop: '8px' },
  notificationsHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', borderBottom: '1px solid #f3f4f6' },
  closeNotificationsBtn: { background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280', padding: '4px' },
  notificationsList: { maxHeight: '300px', overflowY: 'auto' },
  notificationItem: { padding: '12px 16px', borderBottom: '1px solid #f3f4f6', cursor: 'pointer', position: 'relative', transition: 'background-color 0.2s' },
  notificationMessage: { margin: '0 0 4px 0', fontSize: '14px', color: '#374151' },
  notificationTime: { color: '#6b7280', fontSize: '12px' },
  unreadDot: { position: 'absolute', top: '12px', right: '16px', width: '8px', height: '8px', backgroundColor: '#3b82f6', borderRadius: '50%' },
  ...
  searchIcon: { position: 'absolute', left: '12px', color: '#6b7280', zIndex: 1 },
  searchInput: { flex: 1, minWidth: '250px', padding: '12px 12px 12px 40px', border: '2px solid #d1d5db', borderRadius: '8px', fontSize: '14px', outline: 'none', transition: 'all 0.2s' },
  ...
  filterToggle: { display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 16px', backgroundColor: '#f3f4f6', color: '#374151', border: '1px solid #d1d5db', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '500' },
  ...
  emptyState: { textAlign: 'center', padding: '60px 40px', backgroundColor: '#fff', borderRadius: '12px', border: '1px dashed #d1d5db', color: '#6b7280' },
