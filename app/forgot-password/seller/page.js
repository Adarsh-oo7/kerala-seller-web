...
import { Phone, Lock, KeyRound, ArrowLeft, AlertCircle, CheckCircle, Eye, EyeOff, MessageSquare } from 'lucide-react';
...
export default function ForgotPasswordPage() {
    ...
    const handleSendOtp = async (e) => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.post(SEND_RESET_OTP_API, { phone });
            showMessage(`An OTP has been sent to ${phone}. Please check your messages.`, 'success');
            setStep(2);
        } catch (err) {
            const errorMessage = err.response?.data?.error || 
                               err.response?.data?.message || 
                               'Could not send OTP. Please check the phone number and try again.';
            ...
            <div style={styles.container}>
                <div style={styles.card}>
                    ...
                    <div style={styles.footer}>
                        <Link href="/login/seller" style={styles.backLink}>
                            <ArrowLeft size={16} />
                            Back to Login
                        </Link>
                        {step === 2 && (
